...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            // Copies each field to new Path object.
            ...
            // extends copied path (new object) with extendBy
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        PriorityQueue<Path> pathsToEnd = new PriorityQueue<Path>();
        PriorityQueue<Path> discovered = new PriorityQueue<Path>();
        LinkedList<Vertex> visited = new LinkedList<>();
        
        Path originalPath = null;
        Path currentPath = new Path(startVertex);
        discovered.add(currentPath);
        
        while (!currentVertex.edgesLeaving.equals(null)
            || !discovered.isEmpty()) {
            
            System.out.println(currentPath.dataSequence.toString());
            if (discovered.size() == 0) {
                break;
            }
            currentPath = discovered.remove();
            ...
            for (int i = 0; i < currentVertex.edgesLeaving.size(); i++) {
                ...
                for (int j = 0; j < visited.size(); j++) {
                    if (currentVertex.edgesLeaving.get(i).target.equals(visited.get(j))) {
                        numVisited++;
                        wasVisited = true;
                    }
                    
                }
                if (numVisited == currentVertex.edgesLeaving.size()){
                    break;
                    
                }
                if (!wasVisited) {
                    Edge nextEdge = currentVertex.edgesLeaving.get(i);
                    currentPath = new Path(originalPath, nextEdge);
                    visited.add(currentVertex);
                    if (currentPath.end.equals(endVertex)) {
                        pathsToEnd.add(currentPath);
                        System.out.println(pathsToEnd.toString());
                        continue;
                    }
                    discovered.add(currentPath);
        ...
        while (!pathsToEnd.isEmpty()) {
            Path potentialPath = pathsToEnd.remove();
            if ((shortestPath == null) ||
                (potentialPath.compareTo(shortestPath) <= -1)) {
                shortestPath = potentialPath;
            } else if (potentialPath.compareTo(shortestPath) > 0) {
                continue;
            }
        }
